...
function XXXsmarty_function_thumb($params, &$smarty)
	{
	...
	if (empty($params['file'])) return; //{ $smarty->_trigger_fatal_error("thumb: parameter 'file' cannot be empty");return; }
	if (!file_exists($params['file'])) return; //{ $smarty->_trigger_fatal_error("thumb: image file does not exist");return; }
	...
	// UserPictures begin
	$_DST['file']= $params['targetfilename'];
	// UserPictures end
